...
void main() {
  ...
  test("getFlushed", () {
    expect(
      ImmutableSetFlatDelegate({
        ...
        2,
        3,
      }).getFlushed(ImmutableSet.defaultConfig),
      allOf(isA<Set<int>>(), {1, 2, 3}),
    );
  });

  test("Runtime Type", () {
    expect(
      ImmutableSetFlatDelegate([1, 2, 3, 3]),
      isA<ImmutableSetFlatDelegate<int>>(),
    );
  });

  test("unlock", () {
    final ImmutableSetFlatDelegate<int> sFlat = ImmutableSetFlatDelegate([
      1,
      2,
      3,
      3,
  ...
  test("length", () {
    expect(ImmutableSetFlatDelegate([1, 2, 3, 3]).length, 3);
  });

  test("cast", () {
    final sFlatAsNum = ImmutableSetFlatDelegate([1, 2, 3, 3]).cast<num>();
    expect(sFlatAsNum, isA<Iterable<num>>());
  });

  test("iterator", () {
    final Iterator<int?> iter = ImmutableSetFlatDelegate([1, 2, 3, 3]).iterator;
  ...
  test("empty", () {
    final ImmutableSetDelegate<int?> empty = ImmutableSetFlatDelegate.empty();
  ...
  test("deepSetHashCode", () {
    expect(
      ImmutableSetFlatDelegate([1, 2, 3, 3]).deepSetHashcode(),
      SetEquality().hash({1, 2, 3}),
    );
  });
  ...
  test("deepSetEquals", () {
    ...
    expect(sFlat.deepSetEquals(ImmutableSetFlatDelegate<int>({})), isFalse);
    expect(
      sFlat.deepSetEquals(ImmutableSetFlatDelegate<int>({1, 2, 3})),
      isTrue,
    );
    expect(
      sFlat.deepSetEquals(ImmutableSetFlatDelegate<int>([1, 2, 3, 3])),
      isTrue,
    );
    expect(
      sFlat.deepSetEquals(ImmutableSetFlatDelegate<int>([1, 2, 3, 4])),
      isFalse,
    );
  });
  ...
  test("Ensuring Immutability", () {
    ...
    ImmutableSetFlatDelegate<int> sFlat = ImmutableSetFlatDelegate<int>(
      original,
    );
    ...
    original = {1, 2, 3};
    sFlat = ImmutableSetFlatDelegate<int>(original);

    expect(sFlat, <int>{1, 2, 3});

    final ImmutableSetDelegate<int?> s1 = sFlat.add(4);
    final ImmutableSetDelegate<int?> s2 = sFlat.add(3);
    ...
    int willChange = 4;
    ImmutableSetDelegate<int?> s = sFlat.add(willChange);
    ...
    // variable
    original = {1, 2};
    final ImmutableSetFlatDelegate<int> sFlat1 = ImmutableSetFlatDelegate(
          original,
        ),
        sFlat2 = ImmutableSetFlatDelegate(original);
    ...
    original = {1, 2, 3};
    sFlat = ImmutableSetFlatDelegate.unsafe(original);
  ...
  test("join", () {
    ...
    expect(ImmutableSetFlatDelegate(<int>[]).join(","), "");
    expect(ImmutableSetFlatDelegate.empty().join(","), "");
  });
  ...
  test("map", () {
    ...
    expect(ImmutableSetFlatDelegate([1, 2, 3]).map((int? v) => v! + 1), [
      2,
      3,
      4,
    ]);
  ...
  test("reduce", () {
    ...
    expect(
      ImmutableSetFlatDelegate([5]).reduce((int? p, int? e) => p! * (1 + e!)),
      5,
    );
  ...
  test("singleWhere", () {
    // 1) Regular usage
    ImmutableSetFlatDelegate<int> sFlat = ImmutableSetFlatDelegate([
      1,
      2,
      3,
      4,
    ...
    // 2) State exception
    sFlat = ImmutableSetFlatDelegate([1, 2, 3, 4, 5, 6, 5, 6]);
    expect(
      () => sFlat.singleWhere((int? v) => v! < 4, orElse: () => 100),
      throwsStateError,
    );
  });
  ...
  test("whereType", () {
    expect((ImmutableSetFlatDelegate(<num>[1, 2, 1.5]).whereType<double>()), [
      1.5,
    ]);
  });
