...
void main() {
  //
  test("lock", () {
    expect(<int>{}.lock, allOf(isA<ImmutableSet<int>>(), <int>{}));
    expect(<int>{1}.lock, allOf(isA<ImmutableSet<int>>(), {1}));
    expect(<int?>{null}.lock, isA<ImmutableSet<int?>>());
    expect(<int?>{null}.lock, {null});
    expect({1, 2, 3}.lock, allOf(isA<ImmutableSet<int>>(), {1, 2, 3}));
  });

  test("lockUnsafe", () {
    final Set<int> set = {1, 2, 3};
    final ImmutableSet<int> iset = set.lockUnsafe;
  ...
  test("DiffAndIntersectResult.==", () {
    Set<int> set = {1, 2, 3, 4};
    DiffAndIntersectResult setDiffAndIntersect = set.diffAndIntersect({
      5,
      4,
      3,
    });
    ...
    expect(
      setDiffAndIntersect.diffThisMinusOther,
      otherIntersect.diffThisMinusOther,
    );
    expect(
      setDiffAndIntersect.diffOtherMinusThis,
      otherIntersect.diffOtherMinusThis,
    );
    expect(
      setDiffAndIntersect.intersectThisWithOther,
      otherIntersect.intersectThisWithOther,
    );
    expect(
      setDiffAndIntersect.intersectOtherWithThis,
      otherIntersect.intersectOtherWithThis,
    );
    expect(setDiffAndIntersect == otherIntersect, isTrue);

    expect(
      setDiffAndIntersect == <int>{1, 2, 3, 4}.diffAndIntersect({10, 100}),
      isFalse,
    );
  });
  ...
  test("DiffAndIntersectResult.hashCode", () {
    ...
    expect(
      setDiffAndIntersect.hashCode,
      <int>{1, 2, 3, 4}.diffAndIntersect({5, 4, 3}).hashCode,
    );
  });
}
