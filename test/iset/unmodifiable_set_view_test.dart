...
void main() {
  ...
  test("Empty Initialization", () {
    final UnmodifiableSetFromISet<int> unmodifiableSetView =
        UnmodifiableSetFromISet(null);
    expect(unmodifiableSetView.lock, allOf(isA<ImmutableSet<int>>(), <int>{}));
  });

  test("length", () {
    const Set<int> baseSet = {1, 2, 3};
    final UnmodifiableSetFromISet<int> unmodifiableSetView =
            UnmodifiableSetFromISet(baseSet.lock),
    ...
    views.forEach(
      (UnmodifiableSetFromISet<int> view) => expect(view.length, 3),
    );
  });
  ...
  test("lock", () {
    ...
    views.forEach(
      (UnmodifiableSetFromISet<int> view) =>
          expect(view.lock, allOf(isA<ImmutableSet<int>>(), baseSet)),
    );
  });
  ...
  test("toSet", () {
    ...
    views.forEach(
      (UnmodifiableSetFromISet<int> view) => expect(view.toSet(), baseSet),
    );
  });
