...
void main() {
  ...
  test("ordering", () {
    // 1) Regular usage
    ListMapView<String, int> listMapView = ListMapView({
      "a": 1,
      "c": 3,
      "b": 2,
      "d": 4,
  ...
  test("[]=", () {
    final ListMapView<String, int> view = ListMapView({
      "b": 1,
      "a": 2,
      "c": 10,
    });
  ...
  test("addAll", () {
    ...
    // TODO: This is not yet supported, but will be in the future.
    expect(
      () => view.addEntries([MapEntry("a", 3), MapEntry("d", 10)]),
  ...
  test("containsValue", () {
    final ListMapView<String, int> listMapView = ListMapView({
      "a": 1,
      "b": 2,
      "c": 3,
    });
  ...
  test("map", () {
    final ListMap<String, int> listMapView = ListMapView({
      "a": 1,
      "b": 2,
      "c": 3,
      "d": 4,
    ...
    final Map<String, int> mapped = listMapView.map<String, int>(
  ...
  test("removeWhere", () {
    ...
    expect(
      () => view.removeWhere((String key, int value) => key == "a"),
      throwsUnsupportedError,
    );
  });
  ...
  test("update", () {
    ...
    // TODO: This is not yet supported, but will be in the future.
    expect(
      () => view.update("a", (int value) => 2 * value),
      throwsUnsupportedError,
    );
  });
  ...
  test("updateAll", () {
    ...
    // TODO: This is not yet supported, but will be in the future.
    expect(
      () => view.updateAll((String key, int value) => 2 * value),
      throwsUnsupportedError,
    );
  });
