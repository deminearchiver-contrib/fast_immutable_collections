...
void main() {
  ...
  test("of", () {
    // 1) Simple
    ListMap<String, int> listMap = ListMap.of({
      "a": 1,
      "c": 3,
      "b": 2,
      "d": 4,
    ...
    listMap = ListMap.of(
      ...
      sort: true,
      compare: (String keyA, String keyB) => -keyA.compareTo(keyB),
    );
  ...
  test("fromIterables", () {
    ...
    ListMap<String, int> listMap = ListMap.fromIterables(
      keys,
      values,
      sort: false,
    );
  ...
  test("addAll", () {
    ...
    expect(
      () => ListMap.of({"b": 1, "a": 2, "c": 10}).addAll({"a": 3, "d": 10}),
      throwsUnsupportedError,
    );
  });
  ...
  test("addAll", () {
    ...
    expect(
      () => ListMap.of({
        ...
        "a": 2,
        "c": 10,
      }).addEntries([MapEntry("a", 3), MapEntry("d", 10)]),
      throwsUnsupportedError,
    );
  });
  ...
  test("clear", () {
    expect(
      () => ListMap.of({"b": 1, "a": 2, "c": 10}).clear(),
      throwsUnsupportedError,
    );
  });

  test("map", () {
    final ListMap<String, int> listMap = ListMap.of({
      "a": 1,
      "b": 2,
      "c": 3,
      "d": 4,
    ...
    final Map<String, int> mapped = listMap.map<String, int>(
  ...
  test('forEach again', () {
    var listMap = ListMap.of({'b': 1, 'a': 2});
  ...
  test("putIfAbsent", () {
    expect(
      () => ListMap.of({"b": 1, "a": 2, "c": 10}).putIfAbsent("d", () => 10),
      throwsUnsupportedError,
    );
  });
  ...
  test("remove", () {
    expect(
      () => ListMap.of({"b": 1, "a": 2, "c": 10}).remove("d"),
      throwsUnsupportedError,
    );
  });
  ...
  test("updateAll", () {
    ...
    expect(
      () => ListMap.of({
        ...
        "a": 2,
        "c": 10,
      }).updateAll((String key, int value) => 2 * value),
      throwsUnsupportedError,
    );
  });
