...
void main() {
  ...
  test("isEmpty | isNotEmpty", () {
    ...
    final ImmutableMapAddDelegate<String, int> mAdd = ImmutableMapAddDelegate(
      mFlat,
      "d",
      4,
    );
  ...
  test("Ensuring Immutability", () {
    ...
    ImmutableMapAddDelegate<String, int> mAdd = ImmutableMapAddDelegate(
      mFlat,
      "c",
      3,
    );
    ...
    mAdd = ImmutableMapAddDelegate(mFlat, "c", 3);

    expect(mAdd.unlock, <String, int>{"a": 1, "b": 2, "c": 3});

    ImmutableMapDelegate<String, int> m = mAdd.add(key: "d", value: 4);
    ...
    original = {"a": 1, "b": 2};
    mFlat = ImmutableMapFlatDelegate(original);
    final ImmutableMapAddDelegate<String, int> mAdd1 = ImmutableMapAddDelegate(
          mFlat,
          "c",
          3,
        ),
        mAdd2 = ImmutableMapAddDelegate(mFlat, "d", 4);

    expect(mAdd1.unlock, <String, int>{"a": 1, "b": 2, "c": 3});
    expect(mAdd2.unlock, <String, int>{"a": 1, "b": 2, "d": 4});

    m = mAdd1.addAll(ImmutableMap(mAdd2.unlock));
    original.addAll({"z": 5});
  ...
  test("AddAll", () {
    ...
    expect(
      ImmutableMapFlatDelegate({
        ...
        "b": 2,
        "c": 3,
      }).addAll({"a": 1, "b": 8, "d": 10}.lock).entries.asComparableEntries,
      [
        MapEntry("c", 3),
        MapEntry("a", 1),
        MapEntry("b", 8),
