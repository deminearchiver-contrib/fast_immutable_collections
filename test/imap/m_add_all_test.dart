...
void main() {
  ...
  test("isEmpty | isNotEmpty", () {
    ...
    final ImmutableMapFlatDelegate<String, int> mFlatToAdd =
        ImmutableMapFlatDelegate({"b": 2, "c": 3});
    final ImmutableMapAddAllDelegate<String, int> mAddAll =
        ImmutableMapAddAllDelegate.unsafe(mFlat, mFlatToAdd);
  ...
  test("Ensuring Immutability", () {
    ...
    ImmutableMapAddAllDelegate<String, int> mAddAll =
        ImmutableMapAddAllDelegate.unsafe(
          mFlat,
          ImmutableMapFlatDelegate({"c": 3}),
        );
    ...
    mAddAll = ImmutableMapAddAllDelegate.unsafe(
      mFlat,
      ImmutableMapFlatDelegate({"c": 3}),
    );
    ...
    ImmutableMapDelegate<String, int> m = mAddAll.add(key: "d", value: 4);
    ...
    original = {"a": 1, "b": 2};
    mFlat = ImmutableMapFlatDelegate(original);
    final ImmutableMapAddAllDelegate<String, int> mAddAll1 =
            ImmutableMapAddAllDelegate.unsafe(
              mFlat,
              ImmutableMapFlatDelegate({"c": 3}),
            ),
        mAddAll2 = ImmutableMapAddAllDelegate.unsafe(
          mFlat,
          ImmutableMapFlatDelegate({"d": 4}),
        );

    expect(mAddAll1.unlock, <String, int>{"a": 1, "b": 2, "c": 3});
    expect(mAddAll2.unlock, <String, int>{"a": 1, "b": 2, "d": 4});

    m = mAddAll1.addAll(ImmutableMap(mAddAll2.unlock));
    original.addAll({"z": 5});
