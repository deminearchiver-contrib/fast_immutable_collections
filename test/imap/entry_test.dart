...
void main() {
  ...
  test("MapEntryExtension.compareKeyAndValue", () {
    expect(
      MapEntry<String?, int?>(
        null,
        null,
      ).compareKeyAndValue(MapEntry<String?, int?>(null, null)),
      0,
    );
    ...
    expect(
      MapEntry<String, int>(
        "a",
        1,
      ).compareKeyAndValue(MapEntry<String, int>("a", 1)),
      0,
    );
    expect(
      MapEntry<String, int>(
        "a",
        1,
      ).compareKeyAndValue(MapEntry<String, int>("b", 1)),
      -1,
    );
    expect(
      MapEntry<String, int>(
        "a",
        1,
      ).compareKeyAndValue(MapEntry<String, int>("a", 2)),
      -1,
    );
    expect(
      MapEntry<String, int>(
  ...
  test("MapEntryExtension.print", () {
    ...
    final MapEntry<ImmutableList<String>, int> entryWithCollectionKeys =
        MapEntry(["a", "b"].lock, 1);
    ...
    final MapEntry<int, ImmutableSet<int>> entryWithCollectionValues = MapEntry(
      1,
      {1, 2, 3}.lock,
    );
  ...
  test("from", () {
    final Entry<String, int> entry = Entry.from<String, int>(
  ...
  test("compareTo", () {
    expect(
      Entry<String?, int?>(
        null,
        null,
      ).compareTo(Entry<String?, int?>(null, null)),
      0,
    );
    expect(Entry<String, int>("a", 1).compareTo(Entry<String, int>("a", 1)), 0);
    expect(
      Entry<String, int>("a", 1).compareTo(Entry<String, int>("b", 1)),
      -1,
    );
    expect(
      Entry<String, int>("a", 1).compareTo(Entry<String, int>("a", 2)),
      -1,
    );
    expect(
      Entry<String, int>("a", 1).compareTo(Entry<String, int>("b", 2)),
      -1,
    );
    expect(
      Entry<String, int>("a", 2).compareTo(Entry<String, int>("b", 1)),
      -1,
    );
  });
}
