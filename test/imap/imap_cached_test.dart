...
final _reverseKey = CacheKey<ImmutableMap<String, int>, Map<int, String>>(
  (map) => {for (var e in map.entries) e.value: e.key},
);

final _sumValues = CacheKey<ImmutableMap<String, int>, int>(
  (map) => map.values.fold(0, (a, b) => a + b),
);

var _computeCount = 0;

final _countingKey = CacheKey<ImmutableMap<String, int>, Map<String, int>>((
  map,
) {
  _computeCount++;
  return map.unlock;
...
void main() {
  ...
  test("cached | works with const IMap.empty()", () {
    const map = ImmutableMap<String, int>.empty();
    expect(map.cached(_sumValues), 0);
  });
  ...
  test("cached | cache survives flush", () {
    ...
    var map = ImmutableMap<String, int>({"a": 1});
    map = map.add("b", 2);
