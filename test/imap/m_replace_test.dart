...
void main() {
  ...
  test("isEmpty | isNotEmpty", () {
    ...
    final ImmutableMapReplaceDelegate<String, int?> mReplace =
        ImmutableMapReplaceDelegate(mFlat, "a", 2);
  ...
  test("contains", () {
    ...
    ImmutableMapReplaceDelegate<String, int?> mReplace =
        ImmutableMapReplaceDelegate(mFlat, "a", 2);
    ...
    mFlat = ImmutableMapFlatDelegate(originalMap);
    mReplace = ImmutableMapReplaceDelegate(mFlat, "a", 2);
  ...
  test("containsKey", () {
    ...
    mReplace.keys.forEach(
      (String key) => expect(finalMap.containsKey(key), isTrue),
    );
  });
  ...
  test("containsValue", () {
    ...
    expect(
      ImmutableMapReplaceDelegate(
        ImmutableMapFlatDelegate({"a": 1, "b": 2, "c": 3}),
        "a",
        2,
      ).containsValue(2),
      isTrue,
    ...
    final ImmutableMapReplaceDelegate<String, int?> mReplace =
        ImmutableMapReplaceDelegate(mFlat, "a", 2);
    final Map<String, int> finalMap = {"a": 2, "b": 2, "c": 3};
    mReplace.values.forEach(
      (int? value) => expect(finalMap.containsValue(value), isTrue),
    );
  });
  ...
  test("Ensuring Immutability", () {
    ...
    ImmutableMapReplaceDelegate<String, int?> mReplace =
        ImmutableMapReplaceDelegate(mFlat, "b", 4);
    ...
    mReplace = ImmutableMapReplaceDelegate(mFlat, "b", 4);

    expect(mReplace.unlock, <String, int>{"a": 1, "b": 4});

    ImmutableMapDelegate<String, int?> m = mReplace.add(key: "c", value: 3);
    ...
    original = {"a": 1, "b": 2};
    mFlat = ImmutableMapFlatDelegate(original);
    final ImmutableMapReplaceDelegate<String, int?> mReplace1 =
            ImmutableMapReplaceDelegate(mFlat, "b", 4),
        mReplace2 = ImmutableMapReplaceDelegate(mFlat, "b", 5);

    expect(mReplace1.unlock, <String, int>{"a": 1, "b": 4});
    expect(mReplace2.unlock, <String, int>{"a": 1, "b": 5});

    m = mReplace1.addAll(ImmutableMap(mReplace2.unlock));
    original.addAll({"z": 5});
