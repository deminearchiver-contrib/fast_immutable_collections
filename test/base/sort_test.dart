...
void main() {
  ...
  test("sortBy", () {
    ...
    int Function(int, int) compareTo = sortBy(
      (x) => x == 14,
      then: sortBy(
        (x) => x == 15,
        then: sortBy(
  ...
  test("sortLike with mapper", () {
    ...
    // `order is List`
    int Function(String?, String?) compareToList = sortLike(
      const [7, 3, 4, 21, 2],
    ...
    // `order is IList`
    int Function(String?, String?) compareToIList = sortLike(
      [7, 3, 4, 21, 2].lock,
      mapper: (String? text) => text!.length,
      then: sortBy(
        (String? x) => x!.length % 2 == 1,
    ...
    int Function(String?, String?) compareToIterable = sortLike(
      {7, 3, 4, 21, 2},
      mapper: (String? text) => text!.length,
      then: sortBy(
        (String? x) => x!.length % 2 == 1,
  ...
  test("Sort with inconsistencies.", () {
    ImmutableList<int?> list = [3, 4, 9].lock;
