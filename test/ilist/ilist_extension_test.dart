...
void main() {
  ...
  test("lock", () {
    ...
    expect(typedList, isA<ImmutableList<String>>());

    // 3) From a list with one item
    expect([1].lock, isA<ImmutableList<int>>());
    expect([1].lock.isEmpty, isFalse);
    expect([1].lock.isNotEmpty, isTrue);

    // 4) Nulls
    // ignore: prefer_void_to_null
    expect([null].lock, isA<ImmutableList<void>>());
    expect([null].lock.length, 1);
    expect([null].lock[0], null);

    // 5) Typical usage
    expect([1, 2, 3].lock, allOf(isA<ImmutableList<int>>(), [1, 2, 3]));
  });

  test("lockUnsafe", () {
    final List<int> list = [1, 2, 3];
    final ImmutableList<int> ilist = list.lockUnsafe;
