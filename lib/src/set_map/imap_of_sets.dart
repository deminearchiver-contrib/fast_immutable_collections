...
class IMapOfSetsConst<K extends Object?, V extends Object?>
    extends IMapOfSets<K, V> {
  //
...
@immutable
class IMapOfSets<K extends Object?, V extends Object?>
    extends ImmutableCollection<IMapOfSets<K, V>> {
  //
  static ImmutableSetMapConfig get defaultConfig => _defaultConfig;

  static ImmutableSetMapConfig _defaultConfig = const ImmutableSetMapConfig();
  ...
  static set defaultConfig(ImmutableSetMapConfig config) {
  ...
  final ImmutableMap<K, ImmutableSet<V>> _mapOfSets;

  /// The map-of-sets configuration.
  final ImmutableSetMapConfig config;

  /// Returns this `IMapOfSets<K, V>` as an `IMap<K, ISet<V>>`.
  ImmutableMap<K, ImmutableSet<V>> asIMap() => _mapOfSets;
  ...
  IMapOfSets<K, V> get flush {
    _mapOfSets.flush;
    _mapOfSets.values.forEach((s) => s.flush);
    return this;
  }
  ...
  @override
  bool get isFlushed =>
      _mapOfSets.isFlushed && _mapOfSets.values.every((s) => s.isFlushed);
  ...
  @useResult
  static IMapOfSets<K, V> empty<K, V>([ImmutableSetMapConfig? config]) {
    config ??= defaultConfig;
    return IMapOfSets<K, V>._unsafe(
      ImmutableMapImplementation.empty(config.asConfigMap),
      config,
    );
  }
  ...
  const IMapOfSets._(
    ...
    this.config = const ImmutableSetMapConfig(),
  ]);

  /// Create an [IMapOfSets] from a map of sets and a [ImmutableSetMapConfig].
  factory IMapOfSets.withConfig(
    Map<K, Iterable<V>>? mapOfSets,
    ImmutableSetMapConfig config,
  ) {
    final ImmutableSetConfig configSet = config.asConfigSet;
    final ImmutableMapConfig configMap = config.asConfigMap;

    return (mapOfSets == null)
        ? empty<K, V>()
        : IMapOfSets._unsafe(
            ImmutableMap.fromIterables(
              mapOfSets.keys,
              mapOfSets.values.map(
                (value) => ImmutableSet.withConfig(value, configSet),
              ),
              config: configMap,
            ),
            config,
  ...
  /// if not provided, according to the default configuration for [ImmutableSetMapConfig].
  ///
  IMapOfSets.from(
    ImmutableMap<K, ImmutableSet<V>> imap, {
    ImmutableSetMapConfig? config,
  }) : config = config ?? IMapOfSets.defaultConfig,
       _mapOfSets = _setsWithConfig(imap, config ?? IMapOfSets.defaultConfig);

  static ImmutableMap<K, ImmutableSet<V>> _setsWithConfig<K, V>(
    ImmutableMap<K, ImmutableSet<V>> mapOfSets,
    ImmutableSetMapConfig config,
  ) {
  ...
  /// The internal [ImmutableSet] configuration.
  ImmutableSetConfig get configSet => config.asConfigSet;

  /// The internal [ImmutableMap] configuration.
  ImmutableMapConfig get configMap => config.asConfigMap;

  /// See also: [ImmutableSetMapConfig]
  bool get isDeepEquals => config.isDeepEquals;

  /// See also: [ImmutableSetMapConfig]
  bool get isIdentityEquals => !config.isDeepEquals;

  /// Creates a new map-of-sets with the given [config] ([ImmutableSetMapConfig]).
  ///
  ...
  ///
  @useResult
  IMapOfSets<K, V> withConfig(ImmutableSetMapConfig config) {
  ...
  Iterable<MapEntry<K, ImmutableSet<V>>> get entries => _mapOfSets.entries;

  /// Return the [MapEntry] for the given [key].
  /// For key/value pairs that don't exist, it will return `MapEntry(key, null);`.
  MapEntry<K, ImmutableSet<V>?> entry(K key) => _mapOfSets.entry(key);

  /// Return the [MapEntry] for the given [key].
  /// For key/value pairs that don't exist, it will return null.
  MapEntry<K, ImmutableSet<V>>? entryOrNull(K key) =>
      _mapOfSets.entryOrNull(key);
  ...
  ImmutableList<K> keyList({ImmutableListConfig? config}) =>
      ImmutableList.withConfig(keys, config ?? ImmutableList.defaultConfig);

  /// Returns an [Iterable] of the map values.
  Iterable<ImmutableSet<V>> get sets => _mapOfSets.values;

  /// Return all values of all sets, in order, including duplicates.
  Iterable<V> get values sync* {
    for (final ImmutableSet<V> sets in _mapOfSets.values) {
  ...
  /// Returns an [ImmutableList] of the all values in all sets.
  ///
  /// Optionally, you may provide a [config] for the list.
  ///
  ...
  ///
  ImmutableList<V> valueList({ImmutableListConfig? config}) =>
      ImmutableList.withConfig(values, config ?? ImmutableList.defaultConfig);
  ...
  /// Returns an [ImmutableSet] of the [MapEntry]'s.
  ImmutableSet<MapEntry<K, ImmutableSet<V>>> get entriesAsSet =>
      ImmutableSet(entries).withDeepEquals;

  /// Returns an [ImmutableSet] of the keys.
  ImmutableSet<K> get keysAsSet => ImmutableSet(keys).withDeepEquals;

  /// Returns an [ImmutableSet] of the internal sets.
  ImmutableSet<ImmutableSet<V>> get setsAsSet =>
      ImmutableSet(sets).withDeepEquals;

  /// Return all [values] of all [sets], removing duplicates.
  ImmutableSet<V> get valuesAsSet =>
      ImmutableSet.fromIterable(_mapOfSets.entries, mapper: ((e) => e.value));

  /// Return all [keys].
  ImmutableList<K> get keysAsList => ImmutableList(keys).withDeepEquals;

  /// Returns a list of the internal sets.
  ImmutableList<ImmutableSet<V>> get setsAsList =>
      ImmutableList(sets).withDeepEquals;
  ...
  bool isEmptyForKey(K key) {
    final ImmutableSet<V>? values = this[key];
  ...
  IMapOfSets<K, V> add(K key, V value) {
    final ImmutableSet<V> set =
        _mapOfSets[key] ??
        ImmutableSetImplementation<V>.empty(config.asConfigSet);
    final ImmutableSet<V> newSet = set.add(value);
  ...
  IMapOfSets<K, V> addValues(K key, Iterable<V> values) {
    final ImmutableSet<V> set =
        _mapOfSets[key] ??
        ImmutableSetImplementation<V>.empty(config.asConfigSet);
    final ImmutableSet<V> newSet = set.addAll(values);
    return set.same(newSet) ? this : replaceSet(key, newSet);
  }
  ...
  IMapOfSets<K, V> remove(K key, V value) {
    final ImmutableSet<V>? set = _mapOfSets[key];
    if (set == null) return this;
    final ImmutableSet<V> newSet = set.remove(value);
  ...
  ///
  @useResult
  (IMapOfSets<K, V>, int) removeValues(List<V> values) {
    var countRemoved = 0;
    final Map<K, ImmutableSet<V>> map = {};
    for (final MapEntry<K, ImmutableSet<V>> entry in _mapOfSets.entries) {
      final K key = entry.key;
      final ImmutableSet<V> set = entry.value;
      final int setLength = set.length;

      ImmutableSet<V> newSet;
      int newSetLength;
      if (setLength == 0) {
        newSet = set;
        newSetLength = 0;
  ...
  ///
  @useResult
  (IMapOfSets<K, V>, ImmutableSet<V>?) removeValuesFromKeyWhere(
    K key,
    bool Function(V value) test,
  ) => update(key, (set) => set.removeWhere(test));
  ...
  ///
  @useResult
  (IMapOfSets<K, V>, int) removeValuesWhere(
  ...
  ///
  @useResult
  IMapOfSets<K, V> replaceSet(K key, ImmutableSet<V> set) {
  ...
  ///
  @useResult
  IMapOfSets<K, V> clearSet(K key) =>
      replaceSet(key, ImmutableSetImplementation<V>.empty());
  ...
  IMapOfSets<K, V> removeSet(K key) {
    final ImmutableMap<K, ImmutableSet<V>> newMapOfSets = _mapOfSets.remove(
      key,
    );
  ...
  ImmutableSet<V>? getOrNull(K key) => _mapOfSets[key];

  /// Return the [set] for the given [key].
  /// If the [key] doesn't exist, return `null`.
  ImmutableSet<V>? operator [](K key) => _mapOfSets[key];
  ...
  MapEntry<K, ImmutableSet<V>>? getEntryWithValue(V value) {
    for (final MapEntry<K, ImmutableSet<V>> entry in _mapOfSets.entries) {
  ...
  /// Return any `key:set` entry where the value exists in the set.
  /// If that entry doesn't exist, return `null`.
  Set<MapEntry<K, ImmutableSet<V>>> allEntriesWithValue(V value) {
    final Set<MapEntry<K, ImmutableSet<V>>> entriesWithValue = {};
    for (final MapEntry<K, ImmutableSet<V>> entry in _mapOfSets.entries) {
      final set = entry.value;
  ...
  /// ([ImmutableSetMapConfig]) are equal. This may be slow for very large maps, since it compares each
  /// item, one by one.
  @override
  ...
  bool equalItemsToIMap<RK, RV>(ImmutableMap<RK, ImmutableSet<RV>> other) =>
  ...
  int get hashCode {
    return isDeepEquals //
        ? Object.hash(_mapOfSets, config)
        : Object.hash(identityHashCode(_mapOfSets), config.hashCode);
  }
  ...
  ///
  @useResult
  IMapOfSets<K, V> addIMap(ImmutableMap<K, Set<V>> map) {
    return addEntries(map.entries);
  }
  ...
  IMapOfSets<RK, RV> cast<RK, RV>() {
    final ImmutableMap<RK, ImmutableSet<RV>> result = _mapOfSets
        .cast<RK, ImmutableSet<RV>>();
  ...
  IMapOfSets<K, V> clear() {
    if (config.removeEmptySets) {
      ...
      final emptySet = ImmutableSetImplementation<V>.empty(config.asConfigSet);
      return updateAll((key, set) => emptySet);
    }
  }

  /// Applies the function [f] to each element.
  void forEach(void Function(K key, ImmutableSet<V> set) f) =>
      _mapOfSets.forEach(f);
  ...
  IMapOfSets<RK, RV> map<RK, RV>(
    MapEntry<RK, ImmutableSet<RV>> Function(K key, ImmutableSet<V> set)
    mapper, {
    ImmutableSetMapConfig? config,
  }) {
    bool Function(RK key, ImmutableSet<RV> set)? ifRemove;
    if ((config ?? this.config).removeEmptySets) {
      ifRemove = (key, set) => set.isEmpty;
    }
  ...
  IMapOfSets<K, V> removeWhere(
    bool Function(K key, ImmutableSet<V> set) predicate,
  ...
  ///
  @useResult
  (IMapOfSets<K, V>, ImmutableSet<V>?) update(
    K key,
    ImmutableSet<V> Function(ImmutableSet<V> set) update, {
    ImmutableSet<V> Function()? ifAbsent,
  }) {
    bool Function(K key, ImmutableSet<V> set)? ifRemove;
    if (config.removeEmptySets) ifRemove = (key, set) => set.isEmpty;
    final result = _mapOfSets.update(
      key,
      update,
      ifAbsent: ifAbsent,
      ifRemove: ifRemove,
    );
    return (IMapOfSets<K, V>._unsafe(result.$1, config), result.$2);
  }
  ...
  IMapOfSets<K, V> updateAll(
    ImmutableSet<V> Function(K key, ImmutableSet<V> set) update,
  ) {
    bool Function(K key, ImmutableSet<V> set)? ifRemove;
    if (config.removeEmptySets) ifRemove = (key, set) => set.isEmpty;

    return IMapOfSets<K, V>._unsafe(
      _mapOfSets.updateAll(update, ifRemove: ifRemove),
      config,
    );
  }
  ...
  @useResult
  IMapOfSets<V, K> invertKeysAndValues([ImmutableSetMapConfig? config]) {
  ...
  V firstValueWhere(bool Function(V) test, {V Function()? orElse}) {
    for (final ImmutableSet<V> values in _mapOfSets.values) {
