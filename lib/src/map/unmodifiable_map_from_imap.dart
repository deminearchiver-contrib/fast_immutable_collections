...
/// [ImmutableMap] or another [Map]. The construction of the [UnmodifiableMapFromIMap] is very fast,
...
///
/// If you create it from an [ImmutableMap], it is also very fast to lock the [UnmodifiableMapFromIMap]
...
/// [UnmodifiableMapFromIMap] from an [ImmutableMap], then it's totally safe , then it's totally safe
/// because the original [ImmutableMap] can't be modified.
///
/// The only different between an [UnmodifiableMapFromIMap] and an [UnmodifiableMapView] is that
/// [UnmodifiableMapFromIMap] accepts both a [Map] and an [ImmutableMap].
///
/// See also: [ModifiableMapFromIMap]
///
@immutable
class UnmodifiableMapFromIMap<K, V>
    with MapMixin<K, V>
    implements Map<K, V>, CanBeEmpty {
  final ImmutableMap<K, V>? _iMap;
  final Map<K, V>? _map;

  /// Create an unmodifiable [Set] view of type [UnmodifiableSetView], from an [iset].
  UnmodifiableMapFromIMap(ImmutableMap<K, V> imap) : _iMap = imap, _map = null;

  /// Create an unmodifiable [Set] view of type [UnmodifiableSetView], from another [Set].
  UnmodifiableMapFromIMap.fromMap(Map<K, V> map) : _iMap = null, _map = map;

  @override
  void operator []=(K key, V value) =>
      throw UnsupportedError("Map is unmodifiable.");
