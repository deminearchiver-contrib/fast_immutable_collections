...
/// The [ModifiableMapFromIMap] is a safe, modifiable [Map] that is built from an [ImmutableMap].
/// The construction of the map is fast at first, since it makes no copies of the
/// [ImmutableMap] items, but just uses it directly.
///
/// If and only if you use a method that mutates the map, like [add],
/// it will unlock internally (make a copy of all [ImmutableMap] items).
/// This is transparent to you, and will happen at most only once.
/// In other words, it will unlock the [ImmutableMap], lazily, only if necessary.
///
...
///
class ModifiableMapFromIMap<K, V>
    with MapMixin<K, V>
    implements Map<K, V>, CanBeEmpty {
  ImmutableMap<K, V>? _iMap;
  Map<K, V>? _map;

  ModifiableMapFromIMap(ImmutableMap<K, V>? imap)
    : _iMap = imap,
      _map = imap == null ? {} : null;
  ...
  V? operator [](Object? key) =>
      _iMap != null ? _iMap![key as K] : _map![key as K];
  ...
  @override
  void clear() {
    _iMap = ImmutableMapImplementation.empty<K, V>(_iMap?.config);
    _map = null;
  }
