...
/// The [ModifiableFromImmutableList] is a safe, modifiable [List] that is built from an [ImmutableList].
/// The construction of the list is fast at first, since it makes no copies of the
/// [ImmutableList] items, but just uses it directly.
///
/// If and only if you use a method that mutates the list, like [add],
/// it will unlock internally (make a copy of all [ImmutableList] items).
/// This is transparent to you, and will happen at most only once.
/// In other words, it will unlock the [ImmutableList], lazily, only if necessary.
///
...
///
class ModifiableFromImmutableList<T extends Object?>
    with ListMixin<T>
    implements List<T>, CanBeEmpty {
  //
  ImmutableList<T>? _iList;
  List<T>? _list;

  ModifiableFromImmutableList(ImmutableList<T>? ilist)
    : _iList = ilist,
