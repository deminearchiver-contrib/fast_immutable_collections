...
extension FicListExtension<T> on List<T> {
  ...
  T getAndMap(int index, T Function(int index, bool inRange, T? value) map) {
    final inRange = index >= 0 && index < length;
    final value = inRange ? this[index] : null;
  ...
  /// ```
  ///
  List<List<T>> divideList(bool Function(T element) test) {
    if (isEmpty) return [];
  ...
  List<T> concat(
    ...
    return List<T>.filled(totalLength, anyItem)
      ...
      ..setAll(
        list1.length + list2.length + list3.length + list4.length,
        list5,
      );
  }
