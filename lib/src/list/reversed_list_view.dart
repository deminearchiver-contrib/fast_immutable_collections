...
///
class ReversedListView<T extends Object?> implements List<T> {
  const ReversedListView(this.list);
  ...
  void operator []=(int index, T value) =>
      list[list.length - index - 1] = value;
  ...
  void addAll(Iterable<T> iterable) =>
      list.insertAll(0, iterable.toList().reversed);
  ...
  void removeRange(int start, int end) =>
      list.removeRange(length - end, length - start);
  ...
  void setRange(
    ...
    length - end,
    length - start,
    iterable.skip(skipCount).take(end - start).toList().reversed,
  );
  ...
  List<T> sublist(int start, [int? end]) =>
      list.reversed.toList().sublist(start, end);
  ...
  Iterable<T> takeWhile(bool Function(T value) test) =>
      list.reversed.takeWhile(test);

  @override
  List<T> toList({bool growable = true}) =>
      list.reversed.toList(growable: growable);
