...
/// The [ModifiableSetFromISet] is a safe, modifiable [Set] that is built from an [ImmutableSet].
/// The construction of the set is fast at first, since it makes no copies of the
/// [ImmutableSet] items, but just uses it directly.
///
/// If and only if you use a method that mutates the set, like [add],
/// it will unlock internally (make a copy of all [ImmutableSet] items).
/// This is transparent to you, and will happen at most only once.
/// In other words, it will unlock the [ImmutableSet], lazily, only if necessary.
///
...
///
class ModifiableSetFromISet<T extends Object?>
    with SetMixin<T>
    implements Set<T>, CanBeEmpty {
  ImmutableSet<T>? _iSet;
  Set<T>? _set;

  ModifiableSetFromISet(ImmutableSet<T>? iset)
    : _iSet = iset,
      _set = iset == null ? {} : null;
